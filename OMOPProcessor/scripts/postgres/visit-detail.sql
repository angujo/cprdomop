...
WITH vdetails AS (SELECT visit_detail_id, lag(visit_detail_id, 1) over(patition BY patid ORDER BY visit_detail_start_date) AS prev_id FROM {sc}._chunk ch JOIN {sc}.visit_detail on ch.patient_id = patid WHERE ch.ordinal = {ch} ORDER BY patid, visit_detail_start_date asc)
