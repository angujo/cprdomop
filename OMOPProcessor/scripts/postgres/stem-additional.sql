...
ad.patid person_id, null visit_occurrence_id, ad.staffid provider_id, ad.eventdate::timestamp start_datetime, sd.source_concept_id AS concept_id,  ad.source_value, 0 source_concept_id,
32851 AS type_concept_id,  ad.eventdate start_date, NULL operator_concept_id, cu.concept_id unit_concept_id, ad.unit_source_value, ad.eventdate end_date, NULL sig, NULL range_high, NULL range_low,
ad.value_as_number, null value_as_concept_id, ad.qualifier_source_value value_source_value, ad.value_as_string
FROM {sc}._chunk JOIN {sc}.add_in ad on patient_id = ad.patid
join {sc}.source_to_standard sd ON sd.source_code = ad.source_value AND sd.source_vocabulary_id in ('JNJ_CPRD_ADD_ENTTYPE') AND sd.target_standard_concept = 'S' and sd.target_invalid_reason is NULL
