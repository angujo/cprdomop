...
value_as_concept_id::numeric,case when true= textregexeq(value_as_number,'^[[:digit:]]+(\.[[:digit:]]+)?$') then value_as_number::numeric end value_as_number, stem_table.id observation_id, person_id, concept_id observation_concept_id 
