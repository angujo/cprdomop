...
WITH vdetails AS (SELECT visit_detail_id, lag(visit_detail_id, 1) over(partition BY person_id ORDER BY visit_detail_start_date) AS prev_id FROM {sc}.visit_detail WHERE ORDER BY person_id, visit_detail_start_date asc)
